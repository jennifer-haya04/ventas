...
public class DescuentoLogic {
    ...
    private void llenarModelo(List<Descuento> lista, DefaultTableModel modelo){
        
        for(Descuento obj : lista){
            Object data[] = {
                obj.getId_descuento(),
                obj.getNombre(),
                obj.getPorcentaje()
    ...
    public void insertar(Descuento o) throws Exception{
        dao.insertar(o);
    }
    
    public void modificar(Descuento o) throws Exception{
        dao.modificar(o);
    }
    
    public void eliminar(Descuento o) throws Exception{
        dao.eliminar(o);
    }
    
    public DefaultTableModel busqueda(String valor) throws Exception{
        DefaultTableModel modelo = getModelo();
        List<Descuento> lista = dao.busqueda(valor);
        llenarModelo(lista, modelo);
    ...
    public DefaultTableModel paginacion(Integer inicio, Integer fin) throws Exception {
        DefaultTableModel modelo = getModelo();
        List<Descuento> lista = dao.paginacion(inicio, fin);
        llenarModelo(lista, modelo);
    ...
    public void imprimirCB(JComboBox jComboBox) throws Exception{
        ...
        List<Descuento> lista = dao.obtenerTodos();
        for(Descuento o :lista){
            modelo.addElement(o);
        }
        jComboBox.setModel(modelo);
    }
    
    public void selectItemByIDCombo(JComboBox jComboBox, int id_Descuento) throws Exception {
        
        DefaultComboBoxModel modelo = (DefaultComboBoxModel)jComboBox.getModel();
        Descuento obj;
        for(int i=0; i<modelo.getSize();i++){
            obj = (Descuento)modelo.getElementAt(i);
            if(obj.getId_descuento()==id_Descuento){
    ...
    public DefaultComboBoxModel getListaDescuento(String filtro) throws Exception{
        DefaultComboBoxModel modelo = new DefaultComboBoxModel();
        List<Descuento> lista = dao.obtenerTodos();
        
        for(Descuento d:lista){
            if(d.getNombre().contains(filtro)){
                modelo.addElement(d);
            }
        }
        return modelo;
